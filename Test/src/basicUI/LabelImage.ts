...
import { Style } from "./Style";
     ...
     */
     export class LabelImage extends HGroup {
        ...
        private _rollingZoomAlign: string = "center";//放大对齐方式
        ...
        public draw(): void {
            ...
            if (StringUtil.isUsage(s._text)) {
                for (var i = 0; i < s._text.length; ++i) {
                    ...
                    if (texture) {
                        bitmap = ObjectPool.getByClass(Image, "labelimg");
                        s.addChild(bitmap);
                        bitmap.texture = texture;
                        bitmap.width = texture.sourceWidth;
                        bitmap.height = texture.sourceHeight;
                        bitmap.visible = true;
                        bitmap.alpha = 1;
                    }
                }
        ...
        private splitTextureSource(): void {
            ...
            if (s._texture && StringUtil.isUsage(s._chars)) {
                ...
                if (charArr.length > 0) {
                    ...
                    var textureWidth: number = s._texture.sourceWidth;
                    var textureHeight: number = s._texture.sourceHeight;
                    ...
                    for (var i = 0; i < charArr.length; i++) {
                        if (s._horizontalSplit) {
                            s._textureDict[charArr[i]] = Laya.Texture.createFromTexture(s._texture, i * splitWidth + i * s._gapSplit, 0, splitWidth, splitHeight);//spriteSheet.createTexture(s.name + Math.round(Math.random() * 999999) + "_" + charArr[i], i * splitWidth + i * s._gapSplit, 0, splitWidth, splitHeight);
                        } else {
                            s._textureDict[charArr[i]] = Laya.Texture.createFromTexture(s._texture, 0, i * splitHeight + i * s._gapSplit, splitWidth, splitHeight);//spriteSheet.createTexture(s.name + Math.round(Math.random() * 999999) + "_" + charArr[i], 0, i * splitHeight + i * s._gapSplit, splitWidth, splitHeight);
                        }
                    }
                }
            }
        ...
        private onPlaySound(): void {
            if (this._sound == null && StringUtil.isUsage(this._soundName)) {
                this._sound = Laya.loader.getRes(this._soundName);//RES.getRes(this._soundName);
            }
            if (this._sound) {
                this._sound.play();
            }
     ...
     */
     export class EffectNumberRolling {
        ...
        public zoomAlign: string = "center";
        ...
        private onChangeText(): void {
            if (this._rollingText.length > 0) {
                ...
                if (this._isZoom) {
                    ...
                    if (this._labelImg.pivotX == 0) {
                        paramObj["x"] = this._xOld;
                    }
                    if (this._labelImg.pivotY == 0) {
                        paramObj["y"] = this._yOld;
                    }
                    //egret.Tween.get(this._labelImg).to(paramObj, 200);
                    Laya.Tween.to(this._labelImg, paramObj, 200);
                }
                this._isZoom = false;
