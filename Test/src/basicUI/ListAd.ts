...
import { Sprite, Tween } from "./Layout";
...
export class ListAd extends Group {
    ...
    private addItem(index: number): void {
        ...
        if (Math.abs(index - this._middleIndex) <= 1) {
            ...
            item.y = Math.abs(index - this._middleIndex) * this._scaleFactor * item.height * item.pivotY * this._gapFactor + this._middleItemY;
        }
     ...
     */
    public start(): void {
        if (this._delayIndex2) window.clearTimeout(this._delayIndex2); //egret.clearTimeout(this._delayIndex2);
        var oldX: number = 0;
        var oldY: number = 0;
        ...
        for (var i: number = 0; i < moveArray.length; i++) {
            ...
            if (this._direction == "left") {
                ...
                if (i == 0) {
                    Tween.to(this._itemIndexToRender["" + moveArray[i]], {
        ...
        this._delayIndex1 = window.setTimeout(this.refreshTree, this._speed);
    }
    public stop(): void {
        for (var key in this._itemIndexToRender) {
            Tween.clearTween(this._itemIndexToRender[key]);
        }
        if (this._delayIndex1) window.clearTimeout(this._delayIndex1);
        if (this._delayIndex2) window.clearTimeout(this._delayIndex2);
    }
    /**
     * 添加一个新的元素
     * @param type
    ...
    private addNewItem(type: string, index: number = this._middleIndex): void {
        ...
        if (type == "left") {
            Tween.to(item, { x: item.x + this._firstGapWidth, y: item.y + this._scaleFactor * item.height * item.anchorY * this._gapFactor, scaleY: 1 - this._scaleFactor, alpha: 1 - this._scaleFactor }, this._speed)
        } else if (type == "right") {
            Tween.to(item, { x: item.x - this._firstGapWidth, y: item.y + this._scaleFactor * item.height * item.anchorY * this._gapFactor, scaleY: 1 - this._scaleFactor, alpha: 1 - this._scaleFactor }, this._speed)
        }
    }
    ...
    private refreshTree(): void {
        ...
        this._delayIndex2 = window.setTimeout(this.start, this._delay);
    }
